...
angular.module('siTable.directives').directive('siTablePagination', function() {
  return {
    ...
    template: '\
    <ul class="pagination d-flex justify-content-center" ng-show="params.total > params.limit">\
    <li class="page-item" ng-class="{disabled: params.offset === 0}">\
    <a  class="page-link" href ng-click="setPage(1)">{{ params.firstText }}</a>\
    </li>\
    <li class="page-item" ng-class="{disabled: params.offset === 0}">\
    <a class="page-link" href ng-click="setPage(currPage - 1)">{{ params.previousText }}</a>\
    </li>\
    <li  class="page-item" ng-repeat="page in showPages"\
    ng-class="{active: currPage === page}">\
    <a class="page-link" href ng-click="setPage(page)">{{ page }}</a>\
    </li>\
    <li class="page-item" ng-class="{disabled:\
      params.offset + params.limit >= params.total}">\
      <a class="page-link" href ng-click="setPage(currPage + 1)">{{ params.nextText }}</a>\
      </li>\
      <li ng-class="{disabled:\
        params.offset + params.limit >= params.total}">\
        <a class="page-link" href ng-click="setPage(maxPage)">{{ params.lastText }}</a>\
        </li>\
        </ul>',
